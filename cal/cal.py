...
class ModelEvaluation:
  def __init__(self,directory=None,ds='unsw',seed=seed,lab_cat=True,sds=False,fpfn_curve_n_points=None,
               categorical=True,reload_prev=None,methods=['sk','nn'],logf=None,fixed_cutoff=False,
    ...
    self.par_dir=Path(directory)
    self.res_dir=self.par_dir/str(datetime.now()).replace(' ','_')
    ...
    if lab_cat:
      self.Y_labs=list(set(Y_trn)) if isinstance(lab_cat,bool) else list(lab_cat)
      self.Y_trn={l:(Y_trn==l).to_numpy() for l in self.Y_labs}
      self.Y_tst={l:(Y_tst==l).to_numpy() for l in self.Y_labs}
    else:
    ...
    self.regressors={}
    self.benchmarked={}
    self.rs=Resampler(self.X_trn,self.Y_trn,self.par_dir,self.ds_name,
  ...
  def set_targets(self,tfps=None,tfns=None,n_targets=8,min_tfpfn=1.,max_tfpfn=100.,e0=.1):
    if tfps is None:
      ...
      n_targets=len(tfps)
      self.targets={l:[(tfp,tfn,tfp/tfn) for (tfp,tfn) in zip(tfps,tfns)] for\
                    l in self.p_trn}
  ...
  def init_job(self,job):
    ...
    match method:
      ...
      case 'nn':
        from cal.jal import NNPar #optional dependency
        r={l:NNPar(self.seed(),p,self.tfps[l],self.tfns[l],p_resampled=self.rs.get_p(l,rs),init=pm['init'],
                   ...
                   adap_cutoff=pm['adap_cutoff'],fp_fn_weights=pm['fp_fn_weights'])\
           for l,p in self.p_trn.items()}
      case _:
  ...
  def benchmark_job(self,job):
    ...
    for l,reg in regressor.items():
      ...
      r={t:{'trn':{},'tst':{},'tgt':{fpfn:{'fp':fp,'fn':fn} for fp,fn,fpfn in targs}}\
         for t in times}

      for stage,X,Y in [('tst',self.X_tst[l],self.Y_tst[l]),('trn',self.X_trn[l],self.Y_trn[l])]:
        if not self.fixed_cutoff:
  ...
  def res_to_rows(self,job,l,tfpfn):
    ...
    rows=[]
    pmd=tup_to_dict(pmt)
    for t,benchmarked_t in self.benchmarked[job].items():
      res=benchmarked_t[l]
      row=dict(method=method,resampler=resampler,lab_cat=l,p=self.p_trn[l],
               params=pmd,time=t)
      #row['params']['fp_fn_weight']=pmd['fp_fn_weights'][ind_lab]
      row.update({k+lab:res[s][tfpfn][k] for k in ('fp','fn') for (lab,s) in\
                  (('_train','trn'),('_test','tst'),('_target','tgt'))})
      rows.append(row)
    return rows
