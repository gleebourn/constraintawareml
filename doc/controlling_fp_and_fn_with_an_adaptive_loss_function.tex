...
\end{tikzpicture}
\\
is a diagramatic decomposition of\\
\begin{tikzpicture}
  \node(t) {$\boldsymbol\theta$};
...
\begin{tikzpicture}
  ...
  \node[draw, rectangle, thick, minimum height = 1cm,minimum width=2cm] (fpfn)at([xshift=4cm,yshift=-.5cm]f) {$\texttt{moving average}$};
 ...
 \node(D)at([xshift=1cm,yshift=-.5cm]thresh.east){$\Delta$};
  ...
  \path[thick] (f.north -| m) edge["{$(d\texttt{fp},\texttt{dfn})$}",'] (tensor_du -| m);
