...
$$
f_{\boldsymbol\theta}(x)=\tau(\tilde f_{\boldsymbol\theta}(x))=\begin{cases}1~\text{if}~x>\alpha~\text{and}\\0~\text{otherwise.}\end{cases}.
$$
...
\begin{tikzpicture}
  ...
  \node(t) at ([xshift=.5cm]phi){};
  \node(phip)at ([xshift=.5cm]t) {$\boldsymbol\theta'$};
  \node[draw,below=of t, rectangle, thick, minimum height = 2cm,minimum width=2cm] (f) {$(\tilde f,\tilde f_\sharp)$};
  \node[draw,right=of f, rectangle, thick, minimum height = 2cm,minimum width=1cm] (thresh) {$(\tau,\tau_\sharp)$};
  \node[left=of f](x){};
  \node(x_in)at([yshift=.5cm]x){$x$};
...
\begin{tikzpicture}
  ...
  \node[draw, rectangle, thick, minimum height = 1cm,minimum width=2cm] (ad)at([xshift=1cm,yshift=-3cm]t) {$\texttt{reparam}$};
  \node(wp)at([xshift=.5cm]m-|ad){$w'$};
  \node(mp)at([xshift=1cm]wp){$m'$};
  \node(p_wm)at(wp|-tensor_wm){$\Pi$};

  %\node[below=of ad] (t) {};
  \node(tensor_du) at([xshift=.5cm,yshift=-1cm]ad.south) {$\otimes$};

  \node[draw, rectangle, thick, minimum height = 2cm,minimum width=2cm] (f) at([yshift=-7cm]t) {$\texttt{smooth}$};
  \node[draw, rectangle, thick, minimum height = 1cm,minimum width=2cm] (fpfn)at([xshift=4cm,yshift=-.5cm]f) {$\texttt{binadapt}$};

  \node[draw,rectangle, thick, minimum height = 2cm,minimum width=1cm] (thresh) at([xshift=8cm]f) {$\texttt{thresh}$};
  ...
  \node(b)at([xshift=-.5cm]fpfn|-w){$b$};
  \node(bp)at([xshift=1cm]b){$b'$};
  \node(p)at(bp|-tensor_du){$\Pi$};
  ...
  %\path[-latex, thick, auto] ([xshift=.5cm]f.north) to["{$(dfp,dfn)$}",'] ([xshift=.5cm]tensor_du -| f.north) to (tensor_du);
  \path[thick] (f.north -| m) edge["{$(dfp,dfn)$}",'] (tensor_du -| m);
  \path[-latex,thick,auto] (tensor_du -| m) edge (tensor_du);

  \path[-latex, thick, auto] (tensor_du) edge (ad.south-|tensor_du);

  \path[-latex, thick, auto] (b) edge (fpfn.north -| b);
  \path[-latex, thick, auto] (fpfn.north -| bp) edge (p);
  \path[-latex, thick, auto] (p) edge (bp);
